...
import { createDropDownList, openDropDown, closeDropDown } from "./create-dropdown.js";
...
function listenerFormData() {
  ...
  formRef.addEventListener("submit", (e) => {
    ...
    patchRequest(submittedData).then((data) => {
...
async function patchRequest(country) {
  const id = country.id;
  const options = { method: "PATCH", body: JSON.stringify(country), headers: { "content-type": "application/json" } };
  const response = await fetch(BASE_URL + id, options);
  if (response.ok) {
    const result = await response.json();
    const row = document.querySelector(`[data-countryid="${id}"]`);
